...
## Cross-Platform BLE Reference

- `MAC/` build: use `--name` or `--address <CoreBluetooth UUID>`.
- `RBP4/` build: use `--name` or `--address <MAC>` (example `ED:5C:23:84:48:8D`).
- On Raspberry Pi 4, BLE depends on BlueZ (`bluetoothctl`) and the write characteristic UUID `6e400002-b5a3-f393-e0a9-e50e24dcca9e`.

Raspberry Pi 4 quick checks if BLE does not connect:

```bash
bluetoothctl power on
bluetoothctl scan on
# wait a few seconds, confirm your car appears
bluetoothctl devices
bluetoothctl connect ED:5C:23:84:48:8D
```

If the device appears but connect fails, retry once after power-cycling the car.
